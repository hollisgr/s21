...
This project is implementation of the s21_decimal.h library in the C programming language. This library add the ability to work with the "decimal" type, which is not in the language standard. Nevertheless, this type is critically important. For financial calculations, for example, where errors of calculations characteristic of types with floating point are unacceptable. As part of the project I worked with the tasks of processing financial information, dived into the issues of internal representation of different types of data, and solidified knowledge of structured programming.
...
- Created static library named *s21_decimal* (with the s21_decimal.h header file).
- There is a full coverage of library functions code with unit-tests using the Check library.
- Unit tests is cover at least 80% of each function (checked using gcovr).
- Use Makefile for building the library and tests (with targets all, clean, test, s21_decimal.a, report).
- The report target generate a gcovr report in the form of an html page.
