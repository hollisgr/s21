...
## Implementation of the string.h library functions
 ...
 - There is a full coverage of the library's functions by unit-tests using the Check library;
 - Test's code located in src/U-Tests folder;
 - Executable file located in the src/bin folder;
 - Library file located in the src/lib folder;
 - Unit-tests checks the results of implementation by comparing them with the implementation of the standard string.h library;
 - Unit tests cover at least 80% of each function (checked using gcovr);
 - Use Makefile for building the library and tests (with the targets all, clean, test, s21_string.a, report);
 - The report target generate a gcovr report in the form of an html page;
 - Functions worked with z-string made of single-byte characters in ASCII encoding.
