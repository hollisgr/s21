...
The comparison is up to including 7 decimal places.
...
The matrix of algebraic complement looks like this:
...
The determinant can only calculated for a square matrix.
...
- Created static library named *s21_matrix.a* (with the s21_matrix.h header file);
- There is full coverage of library functions code with unit-tests using the Check library;
- Unit tests is cover at least 80% of each function (checked using gcov);  
- Use Makefile for building the library and tests (with targets all, clean, test, s21_matrix.a, report);
